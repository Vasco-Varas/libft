...
void	parse_iscolor_spec(t_gv *gv, char *color)
{
	...
	{
		if(ft_ishexdigit(color[1]))
			if(ft_ishexdigit(color[2]))
				if(ft_ishexdigit(color[3]))
					if(ft_ishexdigit(color[4]))
						if(ft_ishexdigit(color[5]))
							if(ft_ishexdigit(color[6]))
							{
								int col_r = (ft_htoi_char(color[1]) << 4) | ft_htoi_char(color[2]);
								int col_g = (ft_htoi_char(color[3]) << 4) | ft_htoi_char(color[4]);
								int col_b = (ft_htoi_char(color[5]) << 4) | ft_htoi_char(color[6]);
								addstrtobuff(gv, "\x1b[38;2;");
								addstrtobuff(gv, ft_itoa(col_r));
								addstrtobuff(gv, ";");
								addstrtobuff(gv, ft_itoa(col_g));
								addstrtobuff(gv, ";");
								addstrtobuff(gv, ft_itoa(col_b));
								addstrtobuff(gv, "m");
							}
	}
	else
